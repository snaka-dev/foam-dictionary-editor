...
有効な値が実行時解決される C++ クラス階層にあり、その集合がフォークごとに異なりリリースごとに増え、有用な内容が係数名・既定値・OpenFOAM が併記している散文であるもの。これはまさに foamlore のジェネレータが作られた形です。18 リリースにわたる 29 の乱流モデルについて 12,095 行を生成しました。手書きする人はいません。
...
**コストのゲート。** foamlore の `facts/tools/fetch_sources.sh` は各チェックアウトについて `src/TurbulenceModels` / `src/MomentumTransportModels` と `src/finiteVolume/finiteVolume` しかスパースチェックアウトしていません。上記のどのファミリも、全チェックアウトに新しいサブツリーを取得し、そのためのレジストリを書くところから始まります。熱物性ファミリなら一度やる価値がありますが、9 キーのファイルのためにやる価値はありません。この制約を回避するために `schemas/` の生成モジュールを**手編集しないでください**。GENERATED バナーが入っており、テストがそれを検査しています。ジェネレータへの依頼は foamlore 側の仕様書に書きます。
