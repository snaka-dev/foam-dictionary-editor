...
FoDE の Detail ペインは、カーソル位置のキーが何を意味し、どんな値を取り、どのリリースに存在するかを、`schemas/` のスキーマモジュールから説明します。モジュールのない辞書ではペインは空になります。このページは、まだモジュールのない辞書を順位付けし、それぞれについて **FoDE で手書きすべきか**、**[foamlore](foamlore-schema-spec_ja.md) で生成すべきか**を示します。この選択は好みの問題ではなく、辞書の形から決まります。
