...
有効な値が実行時解決される C++ クラス階層にあり、その集合がフォークごとに異なりリリースごとに増え、有用な内容が係数名・既定値・OpenFOAM が併記している散文であるもの。これはまさに foamlore のジェネレータが作られた形です。17 リリースにわたる 29 の乱流モデルについて 11,557 行を生成しました。手書きする人はいません。
