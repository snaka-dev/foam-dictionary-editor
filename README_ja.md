...
- アクションバー下の差分バーに参照パスとカラーレジェンドを表示します。**Clear** をクリックすると比較モードを終了できます
- ツリーオーバーレイ: 薄黄色 = 値の変更、薄青 = このケースのみに存在するキー。ハイライトされた行をホバーするとツールチップに参照値を表示
- ファイル一覧のマーカー: 差分あり → `≠N`（アンバー）、差分なし（確認済み）→ `≠0`（グレー）、未訪問 → マーカーなし。50 件超は `≠50+` に丸めます。マーカーはファイルを開いたときに遅延更新されます
