...
- [FoDE を離れずにケースディレクトリを閲覧](USER_GUIDE_ja.md#ケースナビゲーター) — ファイル一覧の隣の **Cases** タブと、2 ペイン構成の **Case Browser** ウィンドウで、開く・比較するケースを探したり、ファイルマネージャに切り替えずにケース全体を移動・リネーム・複製・削除したりできる。削除はデスクトップのゴミ箱に送られる
