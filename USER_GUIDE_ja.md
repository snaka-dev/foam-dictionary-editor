...
- **マクロ参照** — `nx $nCell;` は 1 レベル深まで解決されます。`nCell` が数値を持つ場合、`nx` はその値を継承します。
...
トップバーの **foamMonitor…** ボタンから `foamMonitor` を起動し、残差などの時系列データを gnuplot でプロットできます。FoDE を離れることなく残差監視が可能です。

### 起動手順

1. ケースを開き、Terminal タブでソルバーを起動します。
2. トップバーの **foamMonitor…** をクリックするとダイアログが開きます。
...
foamMonitor の実行中、トップバーのボタンは **■ foamMonitor** と表示されます。クリックすると foamMonitor を停止し、gnuplot ウィンドウを閉じます。

アイドルタイムアウトが経過した場合（監視ファイルが一定時間更新されなかった場合）も foamMonitor は自動停止し、ボタンが **foamMonitor…** に戻ります。

別のケースを開いた場合も実行中のインスタンスは自動停止されます。

### 注意事項

- foamMonitor は Unix シェルスクリプトです（`foamMonitor` が `PATH` 上にある必要があります）。Windows では動作しません。
