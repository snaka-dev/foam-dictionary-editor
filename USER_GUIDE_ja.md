...
- **Boundary** タブで、全フィールド変数の全境界条件を一覧できます。デフォルトでは行がフィールドファイル、列がパッチですが、**Transpose** チェックボックスで行列を入れ替えられます。セルをダブルクリックして内容を編集、または `–` セルをダブルクリックして新規エントリを作成できます。右クリックメニューでセルの編集・作成・削除・コピー・ペースト・リネームを行えます。パッチの列/行ヘッダーを右クリックすると、そのパッチの境界条件をリネームしたり、全フィールドファイルからまとめて削除したり、全フィールドファイルに新規パッチエントリを追加したりできます。
...
| コントロール | 説明 |
|---|---|
| **Vertices** | メッシュ頂点を赤い球で表示します。 |
| **Vertex labels** | 頂点のインデックス番号をオーバーレイ表示します。 |
| **Block edges** | 六面体ブロックのエッジをワイヤーフレームで表示します。 |
| **Block labels** | 各ヘックスブロックの重心にブロックインデックス番号をオーバーレイ表示します。 |
| **Color blocks** | 各ヘックスブロックを定性的パレット（tab10）の異なる色で表示します。ワイヤーフレームとソリッド面の両方に適用されます。 |
| **Solid blocks** | 半透明のソリッドヘックスブロック面（opacity 0.25）をワイヤーフレームに加えて表示します。**Color blocks** の色設定を共有します。 |
| **Boundary faces** | 境界パッチの面をパッチ種別で色分けして表示します。 |
| **Vertices table** | パネル右側の頂点座標テーブルを表示・非表示にします。 |
| **Refresh** | 現在のツリーからジオメトリを再抽出して再描画します。 |
| **Load STL…** | STL ファイルを読み込み、半透明のグレーオーバーレイとして表示します。複数のファイルを読み込めます。 |
| **Clear STL** | 読み込み済みの STL オーバーレイをすべて削除します。 |
...
- 第 1 ツールバー行の **Vertices table** チェックボックスでテーブルペインを完全に折りたたむことができ、3D ビューの表示領域を広げられます。

### スケール情報・ラベルコントロール（第 2 ツールバー行）

| コントロール | 説明 |
|---|---|
| **Axes** | コーナーに XYZ 方向矢印ウィジェットを表示します。 |
| **Grid** | モデル座標のティックラベル付き座標グリッドを表示します。 |
| **Dimensions** | ワイヤーフレームのバウンディングボックスと X/Y/Z 各方向の範囲を示すオーバーレイを表示します。`convertToMeters` または `scale` が 1 でない場合はスケール係数も表示されます。 |
...
参照ディレクトリを選択すると、アクションバーの下に差分バーが表示されます。差分バーにはカラーレジェンド、参照ケースの名前とフルパス、比較モードを終了する **Clear** ボタンが含まれます。

### ツリーオーバーレイ

比較モードが有効な間、ツリービューは開いているファイルと参照ケース内の対応するファイル（各ケースルートからの相対パスで照合）との差分を背景色で表示します。

| 背景色 | 意味 |
|---|---|
| 薄黄色 | 両ケースにキーが存在するが、値または型が異なる。 |
| 薄青 | このケースにキーが存在するが、参照ケースには存在しない。 |
| 色なし | 両ケースで同一、または比較対象外のノード型（下記参照）。 |

ハイライトされた行をホバーすると、ツールチップに参照値が追記されます。
- **変更あり（薄黄色）** の行では `Ref: <値>` が末尾に付きます。`<値>` は参照ノードの Value 列と同じ書式で表示されます。
...
比較モードが有効な状態でファイルを初めて開いたとき、そのファイルに `≠` サフィックスが付与されます（遅延評価方式 — バックグラウンドスキャンは行いません）。マーカーはファイルを再読み込み・再パースするたびに自動更新されます。
...
差分バーの **Clear** ボタンをクリックして比較モードを終了します。ファイル一覧のすべての `≠` マーカーとツリーのすべての背景色ハイライトが削除されます。**Case > Compare with Case...** を再度選択すると、別の参照ディレクトリに切り替えられます。
