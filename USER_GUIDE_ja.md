...
| ディレクトリを辿って開く・比較するケースを探す | [ケースナビゲーター](#ケースナビゲーター) |
| ケース全体を FoDE から離れずに移動・リネーム・複製・削除する | [Case Browser ウィンドウ](#case-browser-ウィンドウ) |
...
| 寸法が `#include` したファイルにあるジオメトリを表示する | [インクルードファイルに置いたパラメータ](#インクルードファイルに置いたパラメータ) |
...
| ペインを最小化して 3D ビューを広くする | [ペインの最小化](#ペインの最小化) |
...
| 文字を大きくする／ウィンドウが極端に小さく表示されるのを直す | [文字サイズと表示スケーリング](#文字サイズと表示スケーリング) |
...
| 別バージョンの OpenFOAM のケースが動かない理由を調べる | [OpenFOAM のバージョン間の違い](docs/OPENFOAM_VERSIONS_ja.md) |
...
- [ケースナビゲーター](#ケースナビゲーター)
  - [Case Browser ウィンドウ](#case-browser-ウィンドウ)
  - [ケースの移動・リネーム・複製・削除](#ケースの移動リネーム複製削除)
- [ケース比較](#ケース比較)
- [ケースライブラリ](#ケースライブラリ)
- [同梱サンプルケース](#同梱サンプルケース)
- [ケースの再読み込み](#ケースの再読み込み)
...
- [BlockMesh パネル](#blockmesh-パネル)
  - [インクルードファイルに置いたパラメータ](#インクルードファイルに置いたパラメータ)
...
- **左カラム** — ウィンドウ全高で 2 つのタブに分かれています: **Files**（選択中の OpenFOAM ケースのファイル一覧）と **Cases**（ケース全体を探す・管理するためのディレクトリブラウザ。「[ケースナビゲーター](#ケースナビゲーター)」参照）。`Ctrl+1` は前面のタブを問わず、カラム全体を非表示・表示します。
- **右カラム** — 垂直スプリッタで上下に分割。
...
- **▲ Apply Text to Tree** — エディタのテキストを解析し直して上のツリーを再構築します。**Case > Apply Text to Tree** `Ctrl+Shift+A` からも実行できます。
- **▼ Reload from Tree** — 上のツリーからエディタのテキストを生成し直します。**Case > Reload from Tree** からも実行できます（エディタのテキストを上書きするため、ショートカットはありません）。
...
- Case > Case Browser… — 2 ペイン構成の Case Browser ウィンドウを開きます。「[Case Browser ウィンドウ](#case-browser-ウィンドウ)」を参照。
...
パネルツールバーの **Copy Table** ボタンをクリックすると、境界条件テーブル全体をシステムクリップボードにコピーします。ドロップダウンメニューで 3 つの形式を選択できます:
...
- **Copy as CSV** — テーブルの 1 行が 1 レコードとなる CSV を生成します。**Lines per cell** が 2 以上の場合、セル内の各行は `; ` で連結され、改行を含むフィールドは生成されません。通常はこちらを使ってください。Excel へ貼り付ける場合や、仮想マシンのクリップボードをまたぐ場合は特にそうです。アプリケーションは貼り付けられたプレーンテキストを、引用符を解釈する*前*に改行で行に分割するため、セル内の改行はそのままでは余分な行として現れてしまいます。
- **Copy as CSV (multi-line cells)** — RFC 4180 準拠の CSV を生成し、複数行のセル内容を引用符付きフィールド内の実際の改行として保持します。引用符付きフィールドを正しく解釈する貼り付け先（LibreOffice Calc の貼り付けダイアログなど）や、貼り付けではなく `.csv` ファイルとして保存して開く場合に使ってください。

いずれの形式も行ヘッダー列（現在の向きに応じてフィールド変数名またはパッチ名）を含みます。`–` マーカーはそのまま出力されます。
...
寸法やセル数をすべて 1 箇所にまとめ、各辞書からそれを取り込む書き方ができます。

```
#include "settings-region"
```

として `scale`、`x0`、`xMax` などを `system/settings-region` だけに定義する形です。3-D ビューはこれらの定義を、その場に直接書かれていた場合と同じように解決します。したがって全頂点がインクルードファイル由来の `$変数` である `blockMeshDict` も通常どおり表示されます。`topoSetDict`、`snappyHexMeshDict`、`setFieldsDict`、サンプリングの各オーバーレイも同じ仕組みを共有しており、インクルードファイルがさらに別のファイルをインクルードしている場合もたどります。

**インクルードファイルを編集するとビューが更新されます。** `settings-region` の寸法を変更して適用すれば、それをインクルードしている辞書がすべて再描画されます。辞書を開き直す必要はなく、保存も不要です — 未保存の編集は適用した時点で反映されます。

インクルードファイルはツリーには展開されません。またインクルードしている辞書を通してインクルード先へ書き戻されることも一切ありません。`#include` の行は 1 行のままで、ファイル自体を開いて編集するには従来どおり右クリックメニューの **Open Included File** を使います。

インクルードが解決できない場合（ファイルが無い、パースできない）、ビューは辞書が単体で持つ内容にフォールバックします — 従来の動作と同じです。対象の無い `#sinclude` は OpenFOAM として正当な記述であり、問題として扱いません。

`#includeFunc` は意図的に対象外です。これは関数オブジェクトを指しており、そのファイルは既にファイル一覧に現れて単体で描画されるため、ここでも解決すると同じサンプリング線を二重に描くことになります。

`#eval{ … }` の算術は関数を解釈します（`round`、`floor`、`ceil`、`sqrt`、`min`、`max`、`mag`、`pow`、`abs`、三角関数一式、`exp`、`log`、`log10`、`degToRad`、`radToDeg`、定数 `pi`）。そのため `nX #eval{round($xMax/$CS)}` のようなセル数も、黙って飛ばされることなく解決されます。
...
- **`#eval{ expr }`** — 算術式は変数置換後に評価されます。対応演算子：`+`、`−`、`*`、`/`、`**`、括弧。加えて関数 `round`、`floor`、`ceil`、`sqrt`、`min`、`max`、`mag`、`pow`、`abs`、`sin`、`cos`、`tan`、`asin`、`acos`、`atan`、`atan2`、`exp`、`log`、`log10`、`degToRad`、`radToDeg` と定数 `pi` に対応します。例：`zMax #eval{ $length / $nCell };`、`nX #eval{ round($xMax/$CS) };`

`#include` したファイルで定義された変数も解決済みとして扱われます — 「[インクルードファイルに置いたパラメータ](#インクルードファイルに置いたパラメータ)」を参照してください。それでも参照が解決できない場合（名前の綴り間違い、解決できないインクルードなど）、その変数を含む頂点やブロックは静かにスキップされます。したがって想定より頂点の少ないメッシュが表示されたときは、たいてい変数名の綴り間違いです。
...
| **Save Image…** | 3D ビューを表示されているそのままの状態 — 同じピクセルサイズ、同じ背景色 — で PNG / JPEG / BMP / TIFF ファイルに保存します。**形式は Name 欄に入力した拡張子で決まります**。ダイアログのフィルタは形式ごとに分けず 1 つにまとめ、**Image files - PNG, JPEG, BMP, TIFF** という名前にして、対応形式がダイアログ上で読み取れるようにしています — ネイティブの GTK ファイルチューザはフィルタを表示用の絞り込みとしてのみ扱い、名前をそれに合わせて書き換えないため、形式別のドロップダウンだと JPEG を選んだのに名前が `.png` のままで PNG が書き出される、ということが起こりうるからです。書き出せない拡張子は（`.JPG` のような大文字表記の対応形式も含めて）書き込み前に補正され、その補正によって既存のファイルを上書きすることになる場合は確認を求めます（**View** メニューの **Save 3-D View as Image**、および `Ctrl+Shift+I` からも実行できます）。3D ビューを一度も表示していない間は、まだ取り込む画になっていないためグレーアウトします。既定のファイル名は現在のケースディレクトリ内の `<caseName>-3Dview.png` です。**Export Shapes as STL…** と違い、これが保存するのは描画された画そのもの — オーバーレイ・色・カメラアングルを含めた見たままです — であって、ジオメトリそのものではありません。 |
...
| `case_browser_dir` | [ケースナビゲーター](#ケースナビゲーター)と Case Browser が最後に表示していたディレクトリです。両者とも次回起動時にここから再開できるよう、移動のたびに書き込まれます。キーがなければ[デフォルトのケースディレクトリ](#デフォルトのケースディレクトリ設定)が代わりに使われます。**Open Case…** や複製先の起点である `default_case_dir` とは別に保持されます。 |
...
| `openfoam_dir` | [Find OpenFOAM Examples](#find-openfoam-examples) の **Browse…** で選択した OpenFOAM インストールです。この方法で指定した場合のみ書き込まれ、自動検出されたインストールは保存されません。 |
...
3 つのペインをワンクリックで退避させ、同じ操作で元に戻せます。**ファイル一覧**、ツリー右側の **Detail ペイン**、そして下段の **Editor / Terminal** 行です。スプリッタのハンドルをウィンドウ端まで動かしても同じことはできますが、マウス操作が何度も必要で、戻すときにハンドルを探し直すことになります。

方法は 3 つあり、どれも同じ動作です。

- **View メニュー** — **File List** `Ctrl+1`、**Detail Pane** `Ctrl+2`、**Editor / Terminal Pane** `Ctrl+3`。チェックが入っている状態が表示中です。この項目は確実な*復帰手段*でもあります（状態が実行間で保存されるため。後述）。
- **▁ ボタン** — Editor/Terminal タブバー右端。下段専用です。最小化中は **▔** に変わります。
- **スプリッタハンドルのダブルクリック** — ツリーと Detail ペインの間、Editor 行の上、ファイル一覧の右のいずれか。最小化できるペインが隣にないハンドルでは何も起きません。

ファイル一覧と Detail ペインは幅ゼロまで畳まれます。Editor / Terminal 行だけは異なり、ゼロではなく**タブバーの高さ**まで最小化されます。Editor・Terminal タブはクリックでき、**▲ Apply Text to Tree** / **▼ Reload from Tree** もその場に残ります。**▔** を押すと直前の高さに戻ります。

[サイドバイサイドモード](#サイドバイサイドモード)に入ると Detail ペインは自動的に最小化され、抜けると元に戻ります。3D で確認しているファイルに Detail ペインの出番は少なく、その幅は 3D ビューに回したほうがよいためです。手動で先に最小化していた場合は、サイドバイサイドモードを抜けても最小化されたままになります。

どのペインが最小化されているか、そして復帰先のサイズはセッションとして保存されるため、Detail ペインを畳んだまま閉じたウィンドウは次回も同じ状態で開きます。View メニューの項目は常に存在し常に現在の状態を示すので、ペインが画面外に取り残されることはありません。
...
**インストールの選択。** 上部のセレクタには、マシン上で見つかった OpenFOAM インストールが一覧表示されます。FoDE を環境設定済みのシェルから起動した場合はその環境（`WM_PROJECT_DIR`/`FOAM_TUTORIALS`/`FOAM_ETC`）が、さらに `/usr/lib/openfoam/`、`/opt/`、`~/OpenFOAM/` から自動検出されたインストールが新しい順に並びます。**Browse…** で任意のディレクトリ（インストールルートまたは `tutorials/` ディレクトリそのもの）を指定でき、その選択は `app_config.json`（`openfoam_dir` キー）に保存されて次回も初期選択されます。
...
ファイル一覧が表示するのは*1 つの*ケースの中身です。その隣にある **Cases** タブが表示するのはケースそのものです。ディスク上のディレクトリを辿ることで、開く・比較する対象のケースを見つけたり、ファイルマネージャに切り替えることなくケース全体を移動・リネーム・複製・削除したりできます。

`system/` または `constant/` を含む、OpenFOAM ケースらしきディレクトリには **◆** が付き、現在開いているケースであれば太字で表示されます。ダブルクリックすると展開ではなく**ケースを開き**ます。それ以外のフォルダをダブルクリックするとその中へ移動し、**↑** ボタンで 1 つ上へ戻り、**⟳** で現在のフォルダをディスクから再スキャンします。

ナビゲーターが意図的に行わないことが 2 つあります。ケースの中へは降りていきません — ケースの中身を扱うのはファイル一覧の役目であり、1 つのケースを 2 つのブラウザで覗くのは混乱のもとだからです。また、ファイル単体は一切一覧に表示しません — ディレクトリのみです。あるディレクトリが誤ってケースと判定され、その中のフォルダを見る必要がある場合は、右クリックして **Show Subfolders** を選んでください。

ナビゲーターは前回閲覧していた場所を実行間で記憶し、そこから開きます。初回は[デフォルトのケースディレクトリ](#デフォルトのケースディレクトリ設定)を使い、それも無ければホームディレクトリを使います。そこから先はファイルシステム上のどこへでも移動できます。

### Case Browser ウィンドウ

Cases タブ下端の **Browse…**、または **Case > Case Browser…** で、フルサイズのブラウザを開けます。左にフォルダツリー、右に現在のフォルダの中身、下端にアクションの行があります。非モーダルなので作業中もメインウィンドウの横に開いたままにでき、Cases タブと表示場所を共有します — どちらか一方で移動すると、もう一方も追従します。

アクション行は右側ペインで選択中の項目に対して働きます。**Open Case**・**Compare**・**Duplicate…** は実在するケースに対してのみ使用でき、**Move…**・**Rename…**・**Delete…** は任意のフォルダに対して使用でき、**New Folder…** は表示中のフォルダの中に新規フォルダを作成します。

- **Open Case** — **Case > Open Case…** と全く同じように、選択したケースを開きます。
- **Compare** — 選択したケースを[ケース比較](#ケース比較)の参照側にします。比較対象があるためには、あらかじめ何らかのケースを開いておく必要があります。
- **Duplicate…** — 選択したケースをソースとして、通常の[ケースの複製](#ケースの複製)ダイアログを開きます。ケースを複製する際に、全ファイルを持っていくか FoDE が表示するファイルだけにするかを選べます。

### ケースの移動・リネーム・複製・削除

これらは Cases タブの右クリックメニューからも実行できます。それぞれ、簡単には元に戻せない操作を行う前には確認を求め、安全に行えない操作については確認すら求めず端的に拒否します — 拒否はダイアログではなくステータスバーに表示されます。

**削除**すると、そのフォルダと中身すべてがデスクトップのゴミ箱に送られ、通常の方法で復元できます。ゴミ箱が使えない場合 — ゴミ箱ディレクトリを持たない別のファイルシステムだったり、権限の問題があったりする場合 — は理由が示され、完全削除を別の明示的な選択肢として提示します。この 2 段階目の確認なしに完全に削除されることはありません。シンボリックリンクを削除した場合はリンクだけが削除されリンク先はそのまま残り、確認メッセージにもその旨が示されます。

ファイルシステムをまたぐ**移動**はリネームではなくコピー後に削除する形で行われるため、時間がかかることがあります。それが起きる場合は確認メッセージで知らされます。移動が途中で失敗した場合、元のディレクトリは必ず手つかずのまま残り、それがどこにあるか、そして不完全なコピーがどこに残っている可能性があるかの両方が示されます。FoDE が後始末をしようとして唯一残っているコピーを危険にさらすことはありません。

**開いているケースに対する操作**も許可されており、FoDE はそれに追従します。リネームまたは移動すると、ケースは新しい場所で再読み込みされ、ファイル一覧・ツリー・ターミナルの作業ディレクトリもすべて追従します。まず未保存の変更について確認が求められます。開いているケースを削除するとまずそのケースを閉じ、FoDE はケースを開いていない状態になります。

**開いているケースを*含む*フォルダに対する操作は拒否されます。** `~/runs/cavity` を開いた状態で `~/runs` を削除すると、名指ししていないケースを閉じることになってしまうため、FoDE はまずケース自身を閉じるよう求めます。登録済みの[ケースライブラリ](#ケースライブラリ)のディレクトリ、ホームディレクトリ、ファイルシステムのルートも同様に保護されており、ここから変更することはできません。

比較対象にしていたケースが移動された場合は比較もそれに追従し、削除された場合は比較状態が単純にクリアされます。
...
FoDE の文字サイズは、すべてデスクトップのフォント設定から決まります。ツリー、ファイル一覧、Detail ペイン、エディタ、2 種類のターミナルのいずれもです。システムのフォントサイズを大きくすれば、これらは一緒に大きくなります。アプリ側で個別に合わせる設定は必要ありません。それでも足りない場合のために、次の 2 つの手段があります。

### Settings > UI Scale

UI 全体（文字・アイコン・余白のすべて）を **100 %**（既定）、**125 %**、**150 %**、**175 %**、**200 %** の倍率で拡大します。選択内容は `app_config.json` に保存され、再起動後に反映されます。Qt は起動時にスケール係数を確定し、実行中に変更する手段を持たないためです。

FoDE *全体*が他のアプリケーションと比べて小さすぎる場合は、この設定を使ってください。そうなる理由は下の[デスクトップは拡大されているのに FoDE だけ小さい場合](#デスクトップは拡大されているのに-fode-だけ小さい場合)を参照してください。

保存された設定を変えずに試したい場合は、コマンドラインで `--ui-scale` を渡します。`python3 main.py --ui-scale 150` はその 1 回の実行にだけ適用されます。こちらはメニューの 5 段階に限らず、50〜400 の任意の値を受け付けます。

### エディタのズーム

エディタの文字だけを、ウィンドウの他の部分に影響を与えずに拡大縮小できます。

| 操作 | ショートカット |
|------|----------------|
| 拡大 | `Ctrl` + `+`（または `Ctrl` + `=`） |
| 縮小 | `Ctrl` + `-` |
| デスクトップのサイズに戻す | `Ctrl` + `0` |
| マウスでズーム | `Ctrl` + ホイール |

これらのキーはエディタにフォーカスがあるときだけ働くので、ツリーや 3D ビューアで誤って発動することはありません。ズームは固定サイズではなくデスクトップのフォントサイズからの差分として保持されるため（2 段階拡大は、後でフォントサイズを変えても 2 段階拡大のままです）、セッションと一緒に保存され、再起動後も維持されます。

### デスクトップは拡大されているのに FoDE だけ小さい場合

X11 では、Qt はスケール係数を X リソースの `Xft.dpi` だけから決定します。それ以外の仕組みで拡大しているデスクトップ — GDK の `GDK_SCALE` や、Wayland コンポジタが XWayland クライアントに適用するフラクショナルスケーリングなど — では `Xft.dpi` が 96 のままになり、隣の GTK アプリケーションは正しく表示されているのに Qt アプリケーションだけが 1 倍で描画されます。高解像度ディスプレイで FoDE のウィンドウが極端に小さく見える場合、原因はたいていこれです。

対処は 3 通りあり、後のものほど恒久的です。
...
1 つ目は FoDE だけに効き、**Settings > UI Scale** で恒久化できます。2 つ目はその方法で起動した Qt アプリケーション全般に効きます。3 つ目は以後すべての Qt アプリケーションに効き、次回ログイン以降に起動したアプリケーションから反映されます。

次の 2 つも覚えておくと役に立ちます。

- `QT_FONT_DPI=144` は、アイコンや余白は拡大せずに文字だけを大きくします。全体を拡大するより画面に表示できる情報量は多く保てますが、やや詰まった見た目になります。
- `QT_SCALE_FACTOR_ROUNDING_POLICY=PassThrough` は、Qt が小数のスケール係数を整数に丸めるのを防ぎます。125 % のデスクトップが 100 % で表示されてしまうのはこの丸めが原因です。

環境変数の `QT_SCALE_FACTOR` は、保存された **Settings > UI Scale** の値より優先されます。環境変数は目の前のマシンに合わせて設定されたものだからです。`--ui-scale` は両者より優先されます。
