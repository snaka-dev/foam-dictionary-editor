...
`oneBlocks/` with two additional features in `system/blockMeshDict`:

- `system/controlDict`, `system/fvSchemes`, `system/fvSolution`,
  `system/decomposeParDict`, and `constant/transportProperties` are taken
  unchanged from the standard `cavity` case.
- `system/blockMeshDict` is custom-authored (3-D single block), and
...
- `0/U` and `0/p` are custom-authored (same as `oneBlocks/`).
