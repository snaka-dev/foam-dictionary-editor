...
class TestMultiRegionCaseFiles:
    ...
    def test_field_files_in_region_subdir_included(self, tmp_path):
        """Files inside 0/<region>/ are included in the file list."""
        (tmp_path / "0").mkdir()
        (tmp_path / "0" / "fluid").mkdir()
        (tmp_path / "0" / "fluid" / "p").write_text("", encoding="utf-8")
        (tmp_path / "0" / "fluid" / "U").write_text("", encoding="utf-8")
        files = list_case_files(str(tmp_path))
        assert str(tmp_path / "0" / "fluid" / "p") in files
        assert str(tmp_path / "0" / "fluid" / "U") in files

    def test_field_files_direct_and_region_both_included(self, tmp_path):
        """Direct 0/* files and 0/<region>/* files coexist in the list."""
        (tmp_path / "0").mkdir()
        (tmp_path / "0" / "p").write_text("", encoding="utf-8")
        (tmp_path / "0" / "fluid").mkdir()
        (tmp_path / "0" / "fluid" / "p").write_text("", encoding="utf-8")
    ...
    def test_field_region_subdir_orig_included(self, tmp_path):
        """Files inside 0.orig/<region>/ are included."""
        (tmp_path / "0.orig").mkdir()
        (tmp_path / "0.orig" / "solid").mkdir()
        (tmp_path / "0.orig" / "solid" / "T").write_text("", encoding="utf-8")
        files = list_case_files(str(tmp_path))
        assert str(tmp_path / "0.orig" / "solid" / "T") in files

    def test_field_region_no_deeper_recursion(self, tmp_path):
        """Files nested two levels deep (0/<region>/sub/file) are not included."""
        (tmp_path / "0").mkdir()
        (tmp_path / "0" / "fluid").mkdir()
        (tmp_path / "0" / "fluid" / "sub").mkdir()
        (tmp_path / "0" / "fluid" / "sub" / "p").write_text("", encoding="utf-8")
        files = list_case_files(str(tmp_path))
        assert str(tmp_path / "0" / "fluid" / "sub" / "p") not in files
