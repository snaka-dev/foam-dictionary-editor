...
class BoundaryViewPanel(QWidget):
    ...
    def load_case(
        ...
        self._model.load(field_roots, case_dir, available_dirs)
    ...
    def update_field(self, path: str, root: FoamNode) -> None:
        ...
        self._model.update_field(path, root)
        if self._model.case_dir is None:
            return
        selected = self._dir_combo.currentText()
        if not self._model.is_in_dir(path, selected):
            return
    ...
    def _refresh_table(self) -> None:
        ...
        if not selected or not self._model.case_dir:
            self._table.clear()
            self._table.setRowCount(0)
            self._table.setColumnCount(0)
            return

        all_boundaries = self._model.boundaries_for_dir(selected)
        ...
        field_paths = sorted(all_boundaries.keys(), key=lambda p: Path(p).name.lower())
        field_names = [Path(p).name for p in field_paths]

        seen_patches: set[str] = set()
        for bd in all_boundaries.values():
    ...
    def _prompt_add_boundary_field(self) -> None:
        ...
        for root in self._model.field_roots.values():
            existing.update(extract_boundary(root).keys())
