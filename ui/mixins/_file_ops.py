...
class _FileOpsMixin:
    ...
    def _reload_file_list(self) -> None:
        ...
        extra = self.state.case_files_config.get_extra_files() or None
        extra_dirs = self.state.case_files_config.get_extra_dirs() or None
        paths = list_case_files(self.state.current_case_dir, extra, extra_dirs)
        extra_dir_paths = [p for p, _ in extra_dirs] if extra_dirs else None
        self.file_list_panel.load_files(
            paths,
        ...
        try:
            ...
            try:
                ...
                if _parser.errors:
                    n = len(_parser.errors)
                    self.statusBar().showMessage(
                        tr("Saved: {path} — {n} unrecognized {entries}").format(path=self.state.current_file, n=n, entries=("entry" if n == 1 else "entries")),
                        _STATUS_WARNING,
                    )
                else:
                    self.statusBar().showMessage(tr("Saved and parsed: {path}").format(path=self.state.current_file), _STATUS_NORMAL)
            except ParseError as e:
