...
def _group_sort_key(group: str) -> tuple[int, str]:
    """Return (parent_order, sub) so region groups sort after their parent."""
    parts = group.split("/", 1)
...
class FileListPanel(QWidget):
    ...
    def load_files(
        ...
        if extra_files and case_dir:
            # Normalize separators so "system/myDict" matches on Windows too.
            extra_set = {str(Path(ef)) for ef in extra_files}
        ...
        try:
            self._list.clear()
            current_group = None
            for path in self._sorted(paths):
                group = _group_name(path, self._case_dir)
                if group != current_group:
                    is_extra_dir = group in self._extra_dir_set
                    has_unlisted = _has_unlisted_files(
                        case_dir, group, loaded_set, self._extra_dir_set
                    )
                    header = _make_header(group, has_unlisted, is_extra_dir=is_extra_dir)
                    self._list.addItem(header)
                    current_group = group

                p = Path(path)
                is_extra = False
                if extra_set and case_dir:
                    try:
                        rel = str(p.relative_to(Path(case_dir)))
                        is_extra = rel in extra_set
                    except ValueError:
                        pass
                self._list.addItem(_make_item(path, is_extra=is_extra, is_symlink=p.is_symlink()))

            if case_dir:
                time_dirs = detect_time_dirs(case_dir, list(self._extra_dir_set) or None)
                if time_dirs:
                    self._list.addItem(_make_time_dirs_indicator(time_dirs))
        finally:
            self._list.blockSignals(False)

        self._update_extra_indicator(len(extra_set), len(self._extra_dir_set))
    ...
    def _on_context_menu(self, pos) -> None:
        ...
        if group is not None:
            ...
            if chosen == new_action:
                ...
                self.add_file_requested.emit(self._case_dir, group)
    ...
    def _sorted(self, paths: list[str]) -> list[str]:
        return sorted(
            paths,
            key=lambda p: (
                _group_sort_key(_group_name(p, self._case_dir)),
                display_file_name(p).lower(),
            ),
        )
